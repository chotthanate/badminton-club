...
export default function LiffPaymentApp() {
  ...
  function requireSelectedRound() {
    if (selectedPaymentIds.length) return true;
    const message = tr("กรุณาเลือกรอบที่จะชำระเงิน", "Please select a session to pay first.");
    setError(message);
    window.alert(message);
    window.setTimeout(() => document.querySelector(".liff-due-list")?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
    return false;
  }

  async function readSlip(changeEvent) {
    if (!requireSelectedRound()) {
  ...
  async function submitPayment() {
    if (!requireSelectedRound()) return;
    if (!slip) {
    ...
    <PaymentShell language={language} setLanguage={setLanguage}>
      ...
      <section className="liff-payment-card">
        ...
        {availablePayments.length ? <div className="liff-beneficiary-groups">{selectedBeneficiaries.map((entry) => <section className="liff-beneficiary-group" key={entry.id}>
          ...
          <div className="liff-due-list">{entry.payments.map((payment) => <label className={selectedPaymentIds.includes(payment.id) ? "is-selected" : ""} key={payment.id}><input checked={selectedPaymentIds.includes(payment.id)} onChange={() => toggleRound(payment.id)} type="checkbox" /><span><strong>{language === "en" ? formatMemberDate(payment.eventDate, language) : formatThaiDate(payment.eventDate)}</strong><small>{payment.venue}</small>{Number(payment.extrasAmount || 0) > 0 ? <span className="liff-payment-extra-summary"><strong>{tr("ค่าน้ำ/ขนม", "Refreshments")} {baht(payment.extrasAmount)} {tr("บาท", "THB")}</strong>{payment.extras?.length ? payment.extras.map((item, index) => <small key={`${item.name}-${index}`}>{item.name} × {Number(item.quantity || 1)} · {baht(item.amount)} {tr("บาท", "THB")}</small>) : <small>{tr("รายการสินค้ากำลังรอรายละเอียด กรุณาสอบถามแอดมิน", "Item details unavailable; please ask an admin.")}</small>}</span> : null}</span><b>{baht(payment.amount)} {tr("บาท", "THB")}</b></label>)}</div>
      ...
      {availablePayments.length ? <section className="liff-payment-card">
        ...
        <label className={`liff-slip-picker ${slip ? "has-slip" : ""}`}>
          <input accept="image/*" disabled={reading || submitting} onChange={readSlip} onClick={(event) => { if (!requireSelectedRound()) event.preventDefault(); }} type="file" />
        ...
        <button className="liff-payment-submit" disabled={reading || submitting || (selectedPaymentIds.length > 0 && (!slip || recipientStatus === "mismatch" || amountMismatch))} onClick={submitPayment} type="button">{submitting ? tr("กำลังตรวจสอบ...", "Verifying...") : tr(`ยืนยันแจ้งโอน ${baht(total)} บาท`, `Submit payment of ${baht(total)} THB`)}</button>
      </section> : null}
